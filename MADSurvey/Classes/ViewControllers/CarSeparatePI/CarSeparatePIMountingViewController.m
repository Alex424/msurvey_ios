//
//  CarSeparatePIMountingViewController.m
//  MADSurvey
//
...
#import "CarSeparatePIMountingViewController.h"
#import "DataManager.h"

@interface CarSeparatePIMountingViewController ()

@end

@implementation CarSeparatePIMountingViewController
...
- (void) viewWillAppear:(BOOL)animated{
    ...
    self.navigationController.title = @"Car Separated PI Mounting";
    ...
    if ([car.mountSPI isEqualToString:FlushMount]) {
        [comboImageViews[0] setImage:[UIImage imageNamed:@"ic_checkbox_checked"]];
    } else if ([car.mountSPI isEqualToString:SurfaceMount]) {
...
{
    [self performSegueWithIdentifier:UINavigationIDCarSeparatePIMeasure sender:nil];
}
...
-(IBAction)comboAction:(id)sender
{
    ...
    if (btnSelected.tag == 0) {
        car.mountSPI = FlushMount;
    } else {
        car.mountSPI = SurfaceMount;
    }
