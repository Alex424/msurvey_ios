...
#import "LanternReviewViewController.h"
#import "Constants.h"
#import "HelpView.h"
#import "DataManager.h"

@interface LanternReviewViewController () <UITextFieldDelegate>

@end

@implementation LanternReviewViewController
...
- (void) viewWillAppear:(BOOL)animated{
    ...
    self.navigationController.title = @"Lantern Review";
    ...
    descField.text = lantern.lanternDescription ? lantern.lanternDescription : @"";
    mountingField.text = lantern.mount ? lantern.mount : @"";
    wallMaterialField.text = lantern.wallFinish ? lantern.wallFinish : @"";
    ...
    coverDepthField.text = lantern.depth >= 0 ? [formatter stringFromNumber:@(lantern.depth)] : @"";
...
- (IBAction)next:(id)sender {
    NSArray *textFields = @[coverWidthField, coverHeightField, coverDepthField, screwWidthField, screwHeightField, spaceWidthField, spaceHeightField];
    ...
    if (index < textFields.count - 1) {
        ...
        [self performSegueWithIdentifier:UINavigationIDLanternReviewToNotes sender:nil];
    }
}
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    if (textField == mountingField) {
        ...
        [self performSegueWithIdentifier:UINavigationIDLanternReviewToMount sender:nil];
        return NO;
    } else if (textField == wallMaterialField) {
        [DataManager sharedManager].needToGoBack = YES;
        [self performSegueWithIdentifier:UINavigationIDLanternReviewToWall sender:nil];
        return NO;
    } else if (textField == descField) {
        [DataManager sharedManager].needToGoBack = YES;
        [self performSegueWithIdentifier:UINavigationIDLanternReviewToDesc sender:nil];
        return NO;
    }
